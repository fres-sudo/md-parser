...
impl Parser {
    ...
    fn detect_ordered_list_line(line: &str) -> Option<(usize, u32, &str)> {
    ...
    fn detect_continuation_line(line: &str) -> Option<usize> {
        ...
        if Self::detect_list_line(line).is_some() || Self::detect_ordered_list_line(line).is_some()
        {
            return None;
        }
    ...
    fn parse_ordered_list(
        ...
        while i < lines.len() {
            ...
            if let Some((indent_level, _number, content)) = Self::detect_ordered_list_line(line) {
                // Parse the content as inline elements
                let inline_content = if content.is_empty() {
                    Vec::new()
                } else {
                ...
                i += 1;
            } else if let Some((indent_level, _marker, content, checked)) =
                Self::detect_list_line(line)
            {
    ...
    fn collect_blockquote_lines(&self, lines: &[&str], start_idx: usize) -> (String, usize) {
        ...
        while i < lines.len() {
            ...
            if Self::detect_list_line(lines[i]).is_some()
                || Self::detect_ordered_list_line(lines[i]).is_some()
            {
                break;
            }
    ...
    pub fn parse(&mut self) -> Result<Vec<Node>, ParseError> {
        ...
        while i < lines.len() {
            ...
            if Self::detect_ordered_list_line(lines[i]).is_some() {
                let (list_node, new_idx) = self.parse_ordered_list(&lines, i)?;
